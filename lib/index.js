...
export function apply(ctx) {
  ...
  const serve = async (req, res) => {
    try {
      ...
      // 云端写操作（需登录）：订阅/退订专辑、取消收藏声音、取消关注主播。
      // 设置方向的点赞/关注经实测会被风控拦截（需浏览器指纹），只开放纯接口可靠的方向。
      if (pathname === '/dsh-ximalaya/write' && req.method === 'POST') {
        const body = await readBody(req)
        const op = body.op
        const on = body.on !== false // 缺省 true
        const id = Number(body.id)
        if (!Number.isInteger(id) || id <= 0) { writeJson(res, { ok: false, error: '无效的 ID' }, 400); return }
        const handlers = {
          sub: (v) => setSubscriptionAlbum(v, on, cookieHeader()),
          like: (v) => setLikeTrack(v, on, cookieHeader()),
          follow: (v) => setFollow(v, on, cookieHeader()),
        }
        const handler = handlers[op]
        if (handler === undefined) { writeJson(res, { ok: false, error: '不支持的 op（可用：sub / like / follow）' }, 400); return }
        if (cookieHeader() === '') {
          writeJson(res, { ok: false, needLogin: true, error: '请先在面板「我的」页扫码登录，才能同步到云端' }, 401)
          return
        }
        try {
          const result = await handler(id)
          writeJson(res, { ok: true, op, id, on, msg: result.msg })
        } catch (err) {
          writeJson(res, {
            ok: false,
            op, id, on,
