...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const store = {
      ...
      // 「我的」分段：likes 收藏的声音 / anchors 关注的主播 / albums 收藏的专辑
      mineSeg: 'likes',
    ...
    const XMLY_CSS = '' +
      ...
      '.xmly-bar { box-sizing: border-box; display: flex; align-items: center; gap: 8px; width: 100%; max-width: var(--dsh-composer-card-max-width, 780px); margin: 0 auto; padding: 5px 10px 7px; font-size: 12px; color: var(--dsw-alias-label-secondary, #8a8f98); background: var(--dsw-alias-bg-layer-1, rgba(0,0,0,0.04)); border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.2)); border-radius: 8px; cursor: default; user-select: none; position: relative; overflow: hidden; }\n' +
      ...
      '.xmly-progress { position: absolute; left: 0; right: 0; bottom: 0; height: 4px; overflow: hidden; cursor: pointer; background: var(--dsw-alias-bg-layer-2, rgba(128,128,128,0.15)); }\n' +
      ...
      '.xmly-vol-pop { right: 84px; bottom: 30px; width: 170px; }\n' +
      ...
      '.xmly-seg-btn { flex: 1 1 0; background: transparent; border: none; color: var(--dsw-alias-label-secondary, #8a8f98); font-size: 12px; padding: 6px 4px; border-radius: 6px; cursor: pointer; white-space: nowrap; }\n' +
