...
async function getJson(url, { headers = {}, method = 'GET', body: reqBody, timeoutMs = 15000 } = {}) {
  let res
  try {
...
// 网页端 /revision/* 写接口（订阅/点赞/关注）共用的 POST 封装：
// JSON body + cookie 登录态，响应判 ret === 200。
async function postRevisionJson(path, payload, cookie, what) {
  if (!cookie) throw new XmlyError('需要登录后才能' + what, 'NEED_LOGIN', 401)
  ...
  if (body.ret === 401) throw new XmlyError('需要登录后才能' + what + '（或登录态已过期）', 'NEED_LOGIN', 401)
  if (body.ret !== 200) throw new XmlyError(String(body.msg || (what + '失败')), 'API_RET', 502)
  return { msg: String(body.msg || '') }
}
...
function normalizeCover(path) {
  ...
  // 订阅等接口返回裸路径（"storages/..." / "group..."），统一挂到图片 CDN。
  if (/^https:(?!\/\/)/.test(withScheme)) return 'https://imagev2.xmcdn.com/' + withScheme.slice('https:'.length)
...
// /revision/play/v1/show（网页播放器同款）的曲目归一化。
// 该接口不提供付费标记/播放数：isPaid=false、isFree=true（可播性由播放
// 预检 /play 兜底判定），totalCount 由客户端回退专辑详情的 trackCount。
function normalizeShowTrack(t) {
  return {
    id: t.trackId,
    ...
    likes: 0,
    orderNo: t.index || 0,
    cover: normalizeCover(t.trackCoverPath),
    anchorName: '',
    albumTitle: t.albumName || '',
    intro: '',
  }
}
...
export async function albumTracks(albumId, pageId = 1, pageSize = 30) {
  ...
  // 主路：旧版 m 站接口（字段全：付费标记/播放数/评论数）。
  // 部分专辑（多为播客类）已迁走，返回空列表或"已下架"——此时级联到
  // 网页播放器同款 /revision/play/v1/show（xm-sign 签名，列表可用但无付费标记）。
  const primaryErr = await (async () => {
    ...
    try {
      ...
      if (body.ret === 0) {
        ...
        return { ok: false, err: new XmlyError('获取曲目列表失败：该专辑曲目列表为空', 'API_RET', 502) }
      }
      return { ok: false, err: new XmlyError('获取曲目列表失败：' + String(body.msg || body.ret), 'API_RET', 502) }
    } catch (err) {
      return { ok: false, err }
    }
  })()

  if (primaryErr.ok) return primaryErr.result

  // 级联：网页播放器同款接口。
  try {
    const num = Math.max(1, pageId)
    const size = Math.min(50, Math.max(10, pageSize))
    const { body } = await getJson(`${BASE}/revision/play/v1/show?id=${id}&num=${num}&sort=0&size=${size}&ptype=0`, {
      headers: { referer: `${BASE}/album/${id}`, 'xm-sign': getXmSign() },
    })
    if (body.ret === 200) {
      const data = body.data || {}
      const list = Array.isArray(data.tracksAudioPlay) ? data.tracksAudioPlay : []
      return {
        tracks: list.map(normalizeShowTrack),
        // show 接口不给 totalCount：客户端回退用专辑详情的 trackCount。
        maxPageId: data.hasMore ? num + 1 : num,
        totalCount: 0,
        pageId: num,
      }
    }
    throw new XmlyError('获取曲目列表失败：' + String(body.msg || body.ret), 'API_RET', 502)
  } catch {
    // 级联也失败：抛主路错误（信息更贴近原语义）。
    throw primaryErr.err
  }
}
...
function normalizeSubAlbumDoc(doc) {
  ...
  return {
    ...
    anchorName: anchor.anchorNickName || '',
    anchorUid: anchor.anchorUid || 0,
    category: doc.categoryTitle || '',
    score: typeof doc.albumScore === 'string' ? doc.albumScore : '',
    lastTrackTitle: doc.lastUptrackTitle || '',
    lastUpdateText: doc.lastUptrackAtStr || '',
  }
}

/**
 * 我的订阅：当前账号订阅的专辑列表（网页端 /my/subscribed 同款接口，
 * 需登录 cookie；App 内点「订阅」的专辑都会出现在这里）。
 */
export async function mySubscriptions(page = 1, pageSize = 30, cookie) {
  if (!cookie) throw new XmlyError('需要登录后才能查看订阅的专辑', 'NEED_LOGIN', 401)
  const url = `${BASE}/revision/my/getSubscriptionSynthesize?pageNum=${Math.max(1, page)}&pageSize=${Math.min(50, Math.max(1, pageSize))}`
  const { body } = await getJson(url, { headers: { cookie, referer: `${BASE}/my/subscribed` } })
  if (body.ret === 401) throw new XmlyError('需要登录后才能查看订阅的专辑（或登录态已过期）', 'NEED_LOGIN', 401)
  if (body.ret !== 200) throw new XmlyError('获取订阅列表失败：' + String(body.msg || body.ret), 'API_RET', 502)
  const data = body.data || {}
  const list = Array.isArray(data.albumsInfo) ? data.albumsInfo : []
  const total = data.totalCount || list.length
  return {
    albums: list.map(normalizeSubAlbumDoc),
    total,
    page: data.page || page,
    hasMore: list.length > 0 && (data.page || page) * pageSize < total,
  }
}
...
// =====================================================================
// 云端写操作（网页端同款 /revision/* 接口，需登录 cookie）。
// 实测矩阵（2026-08，详见 README「云端同步」）：
//   订阅专辑：set / cancel 均可纯接口调用；
//   点赞声音：cancel 可纯接口；set 会被风控拦截（需浏览器指纹），不建议提供；
//   关注主播：cancel 可纯接口；set 同样被拦截。
// =====================================================================
export async function setSubscriptionAlbum(albumId, on, cookie) {
  const id = Number(albumId)
  if (!Number.isInteger(id) || id <= 0) throw new XmlyError('无效的专辑 ID', 'BAD_REQUEST', 400)
  const path = on ? '/revision/subscription/setSubscriptionAlbum' : '/revision/subscription/cancelSubscriptionAlbum'
  return postRevisionJson(path, { albumId: id }, cookie, on ? '订阅专辑' : '取消订阅')
}

export async function setLikeTrack(trackId, on, cookie) {
  const id = Number(trackId)
  if (!Number.isInteger(id) || id <= 0) throw new XmlyError('无效的曲目 ID', 'BAD_REQUEST', 400)
  const path = on ? '/revision/like/setLikeTrack' : '/revision/like/cancelLikeTrack'
  return postRevisionJson(path, { trackId: id }, cookie, on ? '收藏声音' : '取消收藏')
}

export async function setFollow(uid, on, cookie) {
  const id = Number(uid)
  if (!Number.isInteger(id) || id <= 0) throw new XmlyError('无效的主播 ID', 'BAD_REQUEST', 400)
  const path = on ? '/revision/follow/setFollow' : '/revision/follow/cancelFollow'
  return postRevisionJson(path, { followingUid: id }, cookie, on ? '关注主播' : '取消关注')
}
